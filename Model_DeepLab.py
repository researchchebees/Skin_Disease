...
def DeepLab(sol,pretrained_weights=None, input_size=(128, 128, 1)):
    inputs = Input(input_size)
    conv1 = Conv2D(sol[0], 3, activation='relu', padding='same', kernel_initializer='he_normal')(inputs)
...
def OPT_Deeplab(Activation, pretrained_weights=None, input_size=(128, 128, 1)):
    inputs = Input(input_size)
...
def Train_Deeplab(train_data, train_target, test_data,sol):
    ...
    model.fit(X_train, Y_train, validation_split=0.1, batch_size=5, epochs=sol[1],callbacks=[earlystopper, checkpointer])
    model.save('Deeplab_model.h5')
    pred_img = model.predict(X_test)
    ret_img = pred_img[:, :, :, 0]
    return ret_img

def Test_Deeplab(data):
    # Set some parameters
    IMG_SIZE = 128
    ...
    for i in range(ret_img.shape[0]):
       out.append(ret_img[i].ravel())
    return out



def OPT_Train_deeplab(train_data, train_target, test_data, epoch, Activation):
    # Set some parameters
    IMG_SIZE = 128
    ...
    # Fit model
    model = OPT_Deeplab(Activation)
    earlystopper = EarlyStopping(patience=5, verbose=1)
    checkpointer = ModelCheckpoint('model-dsbowl2018-1.h5', verbose=1, save_best_only=True)
    Results = model.fit(X_train, Y_train, validation_split=0.1, batch_size=16, epochs=epoch,
